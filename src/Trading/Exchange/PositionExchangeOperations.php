...
final class PositionExchangeOperations
{
    ...
    {
        return Exchange::forCanonical($this->position->account->apiSystem->canonical)->mapper();
    }
    ...
    {
        $this->apiProperties = $this->apiMapper()->prepareUpdateMarginTypeProperties($this->position);
        $this->apiProperties->set('account', $this->position->account);
        $this->apiResponse = $this->position->account->withApi()->updateMarginType($this->apiProperties);
    ...
    {
        $this->apiProperties = $this->apiMapper()->prepareUpdateLeverageRatioProperties($this->position, $leverage);
        $this->apiProperties->set('account', $this->position->account);
        $this->apiResponse = $this->position->account->withApi()->changeInitialLeverage($this->apiProperties);
    ...
    {
        $this->apiProperties = $this->apiMapper()->prepareCancelOrdersProperties($this->position);
        $this->apiProperties->set('account', $this->position->account);

        $this->apiResponse = $this->position->account->withApi()->cancelAllOpenOrders($this->apiProperties);
    ...
    {
        $this->apiProperties = $this->apiMapper()->prepareQueryTokenTradesProperties($this->position);
        $this->apiProperties->set('account', $this->position->account);

        $this->apiResponse = $this->position->account->withApi()->accountTrades($this->apiProperties);

        return new ApiResponse(
            response: $this->apiResponse,
            result: $this->apiMapper()->resolveQueryTradeResponse($this->apiResponse, $this->position)
        );
    }
    ...
    {
        $filledQuantity = (string) $this->position->orders()
    ...
    public function apiClose(): ApiResponse
    {
        ...
        if ($order->price !== null && $order->status === 'FILLED') {
            $this->position->updateSaving(['closing_price' => $order->price]);
        }
    ...
    {
        $context = BitgetProductContext::fromQuote($this->position->exchangeSymbol->quote);

        $this->apiProperties = new ApiProperties;
        $this->apiProperties->set('relatable', $this->position);
        $this->apiProperties->set('options.symbol', $this->position->exchangeSymbol->asset);
        ...
        if ($this->position->account->isHedgeMode()) {
            $this->apiProperties->set('options.holdSide', mb_strtolower($this->position->direction));
        }

        $this->apiResponse = $this->position->account->withApi()->flashClosePosition($this->apiProperties);
    ...
    private function setClosingPriceFromBitgetOrder(string $orderId): void
    {
        try {
            ...
            $queryProperties->set('relatable', $this->position);
            $queryProperties->set('options.symbol', $this->position->exchangeSymbol->asset);
            $queryProperties->set('options.productType', $context->productType);
            $queryProperties->set('options.orderId', $orderId);
            $queryProperties->set('account', $this->position->account);

            $orderResponse = $this->position->account->withApi()->getOrderDetail($queryProperties);
            ...
            if ($fillPrice !== null && $fillPrice !== '' && Math::gt((string) $fillPrice, '0')) {
                $this->position->updateSaving(['closing_price' => $fillPrice]);
            }
        } catch (Throwable $e) {
            // Log but don't fail - closing_price is nice to have
            info("Failed to get closing price for BitGet position {$this->position->id}: ".$e->getMessage());
        }
    }
}
