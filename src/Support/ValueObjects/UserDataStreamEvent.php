...
final class UserDataStreamEvent
{
     ...
     * @param  string|null  $side  Exchange order side (BUY / SELL)
     * @param  string|null  $positionSide  Exchange position side (LONG / SHORT / BOTH)
     * @param  int|null  $eventTimeMs  Exchange-claimed event time, milliseconds since epoch
     * @param  bool|null  $reduceOnly  Whether the underlying order carries the reduce-only flag (Binance `o.R`). One-way close detection requires this or closePosition after side checks; Hedge Mode relies on side + positionSide because Binance forbids reduceOnly there. Null for events without this concept.
     * @param  bool|null  $closePosition  Whether the underlying order is a close-position algo (Binance `o.cp`). Distinct from `reduceOnly`: a closePosition=true algo flattens whatever quantity is open at trigger time regardless of stored qty. Set on ALGO_UPDATE events only; null for non-algo frames or when absent.
